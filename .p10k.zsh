# Generated by Powerlevel10k configuration wizard on 2022-10-29 at 03:06 IST.
...
() {
  ...
  function my_git_formatter() {
    ...
    (( VCS_STATUS_COMMITS_BEHIND )) && computed_status+="⇣"
    # ⇡ if ahead of the remote.
    (( VCS_STATUS_COMMITS_AHEAD  )) && computed_status+="⇡"
    # ⇠ if behind the push remote.
    (( VCS_STATUS_PUSH_COMMITS_BEHIND )) && computed_status+="⇠"
    # ⇢ if ahead of the push remote.
    (( VCS_STATUS_PUSH_COMMITS_AHEAD  )) && computed_status+="⇢"
    # * if have stashes.
    (( VCS_STATUS_STASHES        )) && computed_status+="*"
    # 'merge' if the repo is in an unusual state.
    [[ -n $VCS_STATUS_ACTION     ]] && computed_status+="${VCS_STATUS_ACTION}"
    # ~ if have merge conflicts.
    (( VCS_STATUS_NUM_CONFLICTED )) && computed_status+="~"
    # + if have staged changes.
    (( VCS_STATUS_NUM_STAGED     )) && computed_status+="+"
    # ! if have unstaged changes.
    (( VCS_STATUS_NUM_UNSTAGED   )) && computed_status+="!"
    # ? if have untracked files. It's really a question mark, your font isn't broken.
    # See POWERLEVEL9K_VCS_UNTRACKED_ICON above if you want to use a different icon.
    # Remove the next line if you don't want to see untracked files at all.
    (( VCS_STATUS_NUM_UNTRACKED  )) && computed_status+="${(g::)POWERLEVEL9K_VCS_UNTRACKED_ICON}"
    # "─" if the number of unstaged files is unknown. This can happen due to
    # POWERLEVEL9K_VCS_MAX_INDEX_SIZE_DIRTY (see below) being set to a non-negative number lower
    # than the number of files in the Git index, or due to bash.showDirtyState being set to false
    # in the repository config. The number of staged and untracked files may also be unknown
    # in this case.
    (( VCS_STATUS_HAS_UNSTAGED == -1 )) && computed_status+="─"

    if [[ -z $computed_status ]]; then
      computed_status+="✔"
    fi

    typeset -g my_git_format="%5F${res} ${computed_status}"
  }
